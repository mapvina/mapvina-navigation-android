package com.mapvina.navigation.core.snap
 ...
 * uses the MapVina Map SDK, querying the map and snapping the user to the road grid might be a
 * better solution.
 *
 * @since 0.4.0
 */
open class SnapToRoute : Snap() {
     ...
     * creating a [Point] with this distance using [locateAlong].
     *
     *
    ...
    private fun snapLocationBearing(location: Location, routeProgress: RouteProgress): Float? {
        return getCurrentPoint(routeProgress)?.let { currentPoint ->
            getFuturePoint(routeProgress)?.let { futurePoint ->
                ...
                val azimuth = (currentPoint.bearingTo(futurePoint) - Bearing.North).toDouble(Degrees)
    ...
    private fun snapLocationLatLng(location: Location, stepCoordinates: List<Point>): Location {
        ...
        return if (stepCoordinates.size > 1) {
            val stepLineString = LineString(stepCoordinates.map { it.coordinates })
            val pointFeature = stepLineString.nearestPointTo(location.point.coordinates)
    ...
    private fun getCurrentStepPoint(
        ...
        val currentStepLineString = currentLegProgress.currentStep
            .geometry
            .let { geometry ->
                LineString(PolylineEncoding.decode(geometry, Constants.PRECISION_6))
            }
        ...
        return routeProgress.directionsRoute
            ...
            ?.let { firstStep ->
                val currentStepLineString =
                    LineString(PolylineEncoding.decode(firstStep.geometry, Constants.PRECISION_6))
                if (currentStepLineString.coordinates.isEmpty()) {
                    return@let null
                }
