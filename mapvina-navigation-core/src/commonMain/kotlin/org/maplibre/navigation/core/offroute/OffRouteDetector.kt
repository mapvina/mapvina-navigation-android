...
import com.mapvina.navigation.core.utils.RingBuffer
...
import com.mapvina.navigation.core.utils.MeasurementUtils.userTrueDistanceFromStep
import com.mapvina.navigation.core.utils.ToleranceUtils.dynamicOffRouteRadiusTolerance
...
import com.mapvina.spatialk.turf.misc.slice
...
open class OffRouteDetector(
     ...
     * the [MapVinaNavigationOptions.offRouteMinimumDistanceMetersAfterReroute] checked against the last re-route location.
     *
     *
     * Test #3:
     ...
     * But, if the location update is within the [MapVinaNavigationOptions.maneuverZoneRadius] of the
    ...
    override fun isUserOffRoute(
        ...
        options: MapVinaNavigationOptions
    ): Boolean {
    ...
    /**
     * Method to check if the user has passed either the set (in [MapVinaNavigationOptions.offRouteMinimumDistanceMetersAfterReroute])
     ...
     */
    private fun validOffRoute(location: Location, options: MapVinaNavigationOptions): Boolean {
        return lastReroutePoint?.let { lastReroutePoint ->
            ...
            val distanceFromLastReroute =
                distance(lastReroutePoint.coordinates, currentPoint.coordinates).toDouble(Meters)
    ...
    private fun movingAwayFromManeuver(
        ...
        val userPointOnStepFeature = stepLineString.nearestPointTo(currentPoint.coordinates)
        ...
        val remainingStepLineString =
            stepLineString.slice(userPointOnStep, maneuverPoint)
        val userDistanceToManeuver =
            remainingStepLineString.length().toDouble(Meters)
                .toInt()
