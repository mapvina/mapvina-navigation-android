...
import io.github.mapvina.navigation.core.navigation.camera.RouteInformation;
...
import static io.github.mapvina.navigation.core.navigation.NavigationConstants.NAVIGATION_MAX_CAMERA_ADJUSTMENT_ANIMATION_DURATION;
import static io.github.mapvina.navigation.core.navigation.NavigationConstants.NAVIGATION_MIN_CAMERA_TILT_ADJUSTMENT_ANIMATION_DURATION;
import static io.github.mapvina.navigation.core.navigation.NavigationConstants.NAVIGATION_MIN_CAMERA_ZOOM_ADJUSTMENT_ANIMATION_DURATION;

/**
 * Updates the map camera while navigating.
 * <p>
 * This class listens to the progress of {@link MapVinaNavigation} and moves
 * the {@link MapVinaMap} camera based on the location updates.
 *
 * @since 0.6.0
 */
public class NavigationCamera implements LifecycleObserver {
   ...
   *
   * @param mapVinaMap         for moving the camera
   * @param navigation        for listening to location updates
   * @param locationComponent for managing camera mode
   */
  public NavigationCamera(@NonNull MapVinaMap mapVinaMap, @NonNull MapVinaNavigation navigation,
                          @NonNull LocationComponent locationComponent) {
    this.mapVinaMap = mapVinaMap;
    this.navigation = navigation;
    this.locationComponent = locationComponent;
    this.animationDelegate = new CameraAnimationDelegate(mapVinaMap);
   ...
   * @param locationComponent for managing camera mode
   */
  public NavigationCamera(@NonNull MapVinaMap mapVinaMap, LocationComponent locationComponent) {
    this.mapVinaMap = mapVinaMap;
    this.locationComponent = locationComponent;
    this.animationDelegate = new CameraAnimationDelegate(mapVinaMap);
    this.locationComponent.addOnCameraTrackingChangedListener(cameraTrackingChangedListener);
   ...
   */
  NavigationCamera(MapVinaMap mapVinaMap, MapVinaNavigation navigation, ProgressChangeListener progressChangeListener,
  ...
  public void update(NavigationCameraUpdate update) {
    animationDelegate.render(update, MapVinaConstants.ANIMATION_DURATION, null);
  }
   ...
   * the camera stops. During the animation, a call to {@link MapVinaMap#getCameraPosition()} returns an intermediate
   ...
   * @see CameraUpdateMode for how this update interacts with the current tracking
   */
  public void update(NavigationCameraUpdate update, int durationMs, @Nullable MapVinaMap.CancelableCallback callback) {
  ...
  /**
   * This method can be called if you did not pass an instance of {@link MapVinaNavigation}
   * to the constructor.
   * <p>
   ...
   */
  public void addProgressChangeListener(MapVinaNavigation navigation) {
    this.navigation = navigation;
    navigation.setCameraEngine(new DynamicCamera(mapVinaMap));
    navigation.addProgressChangeListener(progressChangeListener);
  }
  ...
  private void initializeWith(MapVinaNavigation navigation) {
    navigation.setCameraEngine(new DynamicCamera(mapVinaMap));
    updateCameraTrackingMode(trackingCameraMode);
  }
  ...
  private void animateCameraForRouteOverview(RouteInformation routeInformation, int[] padding) {
    ...
    if (!routePoints.isEmpty()) {
      animateMapVinaMapForRouteOverview(padding, routePoints);
    }
  }

  private void animateMapVinaMapForRouteOverview(int[] padding, List<Point> routePoints) {
    ...
    mapVinaMap.animateCamera(resetUpdate, 150,
      new CameraOverviewCancelableCallback(overviewUpdate, mapVinaMap)
    );
  }
  ...
  private long getZoomAnimationDuration(double zoom) {
    double zoomDiff = Math.abs(mapVinaMap.getCameraPosition().zoom - zoom);
  ...
  private long getTiltAnimationDuration(double tilt) {
    double tiltDiff = Math.abs(mapVinaMap.getCameraPosition().tilt - tilt);
    return (long) MathUtils.clamp(
      500 * tiltDiff,
