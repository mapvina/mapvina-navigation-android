...
import io.github.mapvina.navigation.core.route.RouteFetcher;
 ...
 */
public class MapVinaRouteFetcher extends RouteFetcher {
    ...
    public MapVinaRouteFetcher(Context context) {
    ...
    public NavigationRoute.Builder buildRequest(Location location, RouteProgress progress) {
        ...
        List<Point> remainingWaypoints = toJvmPoints(routeUtils.calculateRemainingWaypoints(progress));
     ...
     * any {@link RouteListener} that has been added via {@link MapVinaRouteFetcher#addRouteListener(RouteListener)}.
     *
    ...
    private Callback<DirectionsResponse> directionsResponseCallback = new Callback<>() {
        ...
        public void onResponse(@NonNull Call<DirectionsResponse> call,
             ...
             */
            DirectionsResponse mapvinaResponse = DirectionsResponse.fromJson(response.body().toJson());
            updateListeners(mapvinaResponse, routeProgress);
        }
    ...
    private List<Point> toMapboxPointList(List<io.github.mapvina.spatialk.geojson.Point> pointList) {
        List<Point> mapboxPointList = new ArrayList<>();
        for (io.github.mapvina.spatialk.geojson.Point point : pointList) {
            mapboxPointList.add(toMapboxPoint(point));
        }
        return mapboxPointList;
    }

    private Point toMapboxPoint(io.github.mapvina.spatialk.geojson.Point point) {
        return Point.fromLngLat(point.getLongitude(), point.getLatitude());
    }

    private List<io.github.mapvina.spatialk.geojson.Point> toMapVinaPointList(List<Point> pointList) {
        List<io.github.mapvina.spatialk.geojson.Point> mapboxPointList = new ArrayList<>();
        for (Point point : pointList) {
            mapboxPointList.add(toMapVinaPoint(point));
        }
        return mapboxPointList;
    }

    private io.github.mapvina.spatialk.geojson.Point toMapVinaPoint(Point point) {
        return new io.github.mapvina.spatialk.geojson.Point(point.longitude(), point.latitude());
    }
}
