...
import static io.github.mapvina.navigation.core.location.LocationExtKt.toAndroidLocation;
import static io.github.mapvina.navigation.core.navigation.NavigationConstants.NAVIGATION_MINIMUM_MAP_ZOOM;
import static io.github.mapvina.navigation.android.navigation.ui.v5.GeoJsonExtKt.toJvm;
...
import io.github.mapvina.navigation.android.navigation.ui.v5.route.OnRouteSelectionChangeListener;
...
import io.github.mapvina.navigation.android.navigation.ui.v5.ThemeSwitcher;
...
/**
 * Wrapper class for {@link MapVinaMap}.
 * <p>
 ...
 */
public class NavigationMapVinaMap {
   ...
   * @param mapVinaMap for APIs to interact with the map
   */
  public NavigationMapVinaMap(@NonNull MapView mapView, @NonNull MapVinaMap mapVinaMap) {
    this.mapView = mapView;
    this.mapVinaMap = mapVinaMap;
    initializeLocationComponent(mapView, mapVinaMap);
    initializeMapPaddingAdjustor(mapView, mapVinaMap);
    initializeNavigationSymbolManager(mapView, mapVinaMap);
    initializeMapLayerInteractor(mapVinaMap);
    initializeRoute(mapView, mapVinaMap);
    initializeCamera(mapVinaMap, locationComponent);
    initializeLocationFpsDelegate(mapVinaMap, locationComponent);
  }

  // Package private (no modifier) for testing purposes
  NavigationMapVinaMap(MapLayerInteractor layerInteractor) {
    this.layerInteractor = layerInteractor;
  }

  // Package private (no modifier) for testing purposes
  NavigationMapVinaMap(LocationComponent locationComponent) {
    this.locationComponent = locationComponent;
  }

  // Package private (no modifier) for testing purposes
  NavigationMapVinaMap(NavigationMapRoute mapRoute) {
    this.mapRoute = mapRoute;
  }

  // Package private (no modifier) for testing purposes
  NavigationMapVinaMap(NavigationSymbolManager navigationSymbolManager) {
    this.navigationSymbolManager = navigationSymbolManager;
  }

  // Package private (no modifier) for testing purposes
  NavigationMapVinaMap(@NonNull MapWayName mapWayName, @NonNull MapFpsDelegate mapFpsDelegate) {
  ...
  // Package private (no modifier) for testing purposes
  NavigationMapVinaMap(MapVinaMap mapVinaMap, MapLayerInteractor layerInteractor, MapPaddingAdjustor adjustor) {
    this.layerInteractor = layerInteractor;
    initializeWayName(mapVinaMap, adjustor);
  }
   ...
   * @deprecated Use {@link NavigationMapVinaMap#addDestinationMarker(Point)} instead.
   * A {@link Context} is no longer needed.
   */
  @Deprecated
  public void addMarker(Context context, Point position) {
    navigationSymbolManager.addDestinationMarkerFor(toJvm(position));
  }
   ...
   * <p>
   * Please note, the map will manage all markers added.  Calling {@link NavigationMapVinaMap#clearMarkers()}
   * will clear all destination / custom markers that have been added to the map.
   *
   * @param options for the custom {@link io.github.mapvina.android.plugins.annotation.Symbol}
   */
   ...
   * <p>
   * These will automatically be removed in {@link MapVinaNavigation#onDestroy()}.
   *
   * @param navigation to add the progress listeners
   */
  public void addProgressChangeListener(@NonNull MapVinaNavigation navigation) {
    initializeWayName(mapVinaMap, mapPaddingAdjustor);
   ...
   * <p>
   * This method uses {@link NavigationMapVinaMapInstanceState}, stored with the provided key.  This key
   * can also later be used to extract the {@link NavigationMapVinaMapInstanceState}.
   *
  ...
  public void saveStateWith(String key, Bundle outState) {
    ...
    NavigationMapVinaMapInstanceState instanceState = new NavigationMapVinaMapInstanceState(settings);
    outState.putParcelable(key, instanceState);
  }

  /**
   * Can be used to restore a {@link NavigationMapVinaMap} after it has been initialized.
   * <p>
   ...
   * <p>
   * You can extract the {@link NavigationMapVinaMapInstanceState} in <tt>onRestoreInstanceState</tt> and then
   * restore the map once it's ready.
   *
   * @param instanceState to extract state variables
   */
  public void restoreFrom(NavigationMapVinaMapInstanceState instanceState) {
  ...
  /**
   * Provides the {@link MapVinaMap} originally given in the constructor.
   * <p>
   * This method gives access to all map-related APIs.
   *
   * @return map provided in the constructor
   */
  public MapVinaMap retrieveMap() {
    return mapVinaMap;
  }
  ...
  private void initializeLocationComponent(MapView mapView, MapVinaMap map) {
  ...
  private void initializeMapPaddingAdjustor(MapView mapView, MapVinaMap mapVinaMap) {
    mapPaddingAdjustor = new MapPaddingAdjustor(mapView, mapVinaMap);
  }

  private void initializeNavigationSymbolManager(MapView mapView, MapVinaMap mapVinaMap) {
    Bitmap markerBitmap = ThemeSwitcher.retrieveThemeMapMarker(mapView.getContext());
    mapVinaMap.getStyle().addImage(NavigationSymbolManager.MAPVINA_NAVIGATION_MARKER_NAME, markerBitmap);
    SymbolManager symbolManager = new SymbolManager(mapView, mapVinaMap, mapVinaMap.getStyle());
    navigationSymbolManager = new NavigationSymbolManager(symbolManager);
    SymbolOnStyleLoadedListener onStyleLoadedListener = new SymbolOnStyleLoadedListener(mapVinaMap, markerBitmap);
    mapView.addOnDidFinishLoadingStyleListener(onStyleLoadedListener);
  }

  private void initializeMapLayerInteractor(MapVinaMap mapVinaMap) {
    layerInteractor = new MapLayerInteractor(mapVinaMap);
  }

  private void initializeRoute(MapView mapView, MapVinaMap map) {
    Context context = mapView.getContext();
  ...
  private void initializeCamera(MapVinaMap map, LocationComponent locationComponent) {
    mapCamera = new NavigationCamera(map, locationComponent);
  }

  private void initializeLocationFpsDelegate(MapVinaMap map, LocationComponent locationComponent) {
    locationFpsDelegate = new LocationFpsDelegate(map, locationComponent);
  }

  private void initializeWayName(MapVinaMap mapVinaMap, MapPaddingAdjustor paddingAdjustor) {
    if (mapWayName != null) {
      return;
    }
    initializeStreetsSource(mapVinaMap);
  ...
  private void initializeStreetsSource(MapVinaMap mapVinaMap) {
  ...
  private void updateMapWayNameWithLocation(Location location) {
    ...
    PointF mapPoint = mapVinaMap.getProjection().toScreenLocation(latLng);
