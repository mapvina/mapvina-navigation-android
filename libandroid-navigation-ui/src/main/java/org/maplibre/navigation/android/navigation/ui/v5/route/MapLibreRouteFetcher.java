...
import com.mapvina.navigation.core.route.RouteFetcher;
...
public class MapVinaRouteFetcher extends RouteFetcher {
    ...
    public NavigationRoute.Builder buildRequest(Location location, RouteProgress progress) {
        ...
        NavigationRoute.Builder builder = NavigationRoute.builder(context)
                .origin(toMapVinaPoint(origin), bearing, BEARING_TOLERANCE)
    ...
    private void addDestination(List<Point> remainingWaypoints, NavigationRoute.Builder builder) {
        if (!remainingWaypoints.isEmpty()) {
            builder.destination(toMapVinaPoint(retrieveDestinationWaypoint(remainingWaypoints)));
        }
    }
    ...
    private void addWaypoints(List<Point> remainingCoordinates, NavigationRoute.Builder builder) {
        if (!remainingCoordinates.isEmpty()) {
            for (Point coordinate : remainingCoordinates) {
                builder.addWaypoint(toMapVinaPoint(coordinate));
            }
        }
    }
    ...
    private List<Point> toMapboxPointList(List<com.mapvina.geojson.Point> pointList) {
        List<Point> mapboxPointList = new ArrayList<>();
        for (com.mapvina.geojson.Point point : pointList) {
            mapboxPointList.add(toMapboxPoint(point));
        }
        return mapboxPointList;
    }

    private Point toMapboxPoint(com.mapvina.geojson.Point point) {
        return Point.fromLngLat(point.longitude(), point.latitude());
    }

    private List<com.mapvina.geojson.Point> toMapVinaPointList(List<Point> pointList) {
        List<com.mapvina.geojson.Point> mapboxPointList = new ArrayList<>();
    ...
    private com.mapvina.geojson.Point toMapVinaPoint(Point point) {
        return com.mapvina.geojson.Point.fromLngLat(point.longitude(), point.latitude());
    }
}
