...
public class NavigationMapRoute implements LifecycleObserver {
   ...
   * @param mapVinaMap the MapVinaMap to apply route with
   * @since 0.4.0
   */
  public NavigationMapRoute(@NonNull MapView mapView, @NonNull MapVinaMap mapVinaMap) {
    this(null, mapView, mapVinaMap, R.style.NavigationMapRoute);
  }
  ...
  public NavigationMapRoute(@NonNull MapView mapView, @NonNull MapVinaMap mapVinaMap,
    ...
    this(null, mapView, mapVinaMap, R.style.NavigationMapRoute, belowLayer);
  }

  /**
   * Construct an instance of {@link NavigationMapRoute}.
   *
   * @param navigation an instance of the {@link MapVinaNavigation} object. Passing in null means
   *                   your route won't consider rerouting during a navigation session.
   * @param mapView    the MapView to apply the route to
   * @param mapVinaMap  the MapVinaMap to apply route with
   * @since 0.4.0
   */
  public NavigationMapRoute(@Nullable MapVinaNavigation navigation, @NonNull MapView mapView,
                            @NonNull MapVinaMap mapVinaMap) {
    this(navigation, mapView, mapVinaMap, R.style.NavigationMapRoute);
  }
   ...
   */
  public NavigationMapRoute(@Nullable MapVinaNavigation navigation, @NonNull MapView mapView,
                            @NonNull MapVinaMap mapVinaMap, @Nullable String belowLayer) {
    this(navigation, mapView, mapVinaMap, R.style.NavigationMapRoute, belowLayer);
  }
   ...
   */
  public NavigationMapRoute(@Nullable MapVinaNavigation navigation, @NonNull MapView mapView,
                            @NonNull MapVinaMap mapVinaMap, @StyleRes int styleRes) {
    this(navigation, mapView, mapVinaMap, styleRes, null);
  }
  ...
  public NavigationMapRoute(@Nullable MapVinaNavigation navigation, @NonNull MapView mapView,
    ...
    this.routeLine = buildMapRouteLine(mapView, mapVinaMap, styleRes, belowLayer);
    this.routeArrow = new MapRouteArrow(mapView, mapVinaMap, styleRes);
  ...
  // For testing only
  NavigationMapRoute(@Nullable MapVinaNavigation navigation, @NonNull MapView mapView,
                     @NonNull MapVinaMap mapVinaMap, @StyleRes int styleRes, @Nullable String belowLayer,
   ...
   * the progress updates from {@link MapVinaNavigation}.
   * <p>
   * If a new route is given to {@link MapVinaNavigation#startNavigation(DirectionsRoute)}, this
  ...
  /**
   * Should be called if {@link NavigationMapRoute#addProgressChangeListener(MapVinaNavigation)} was
   * called to prevent leaking.
   *
   * @param navigation to remove the progress change listener
   */
  public void removeProgressChangeListener(MapVinaNavigation navigation) {
    if (navigation != null) {
  ...
  private MapRouteLine buildMapRouteLine(@NonNull MapView mapView, @NonNull MapVinaMap mapVinaMap,
    ...
    return new MapRouteLine(context, mapVinaMap.getStyle(), styleRes, belowLayer,
  ...
  private void addListeners() {
    if (!isMapClickListenerAdded) {
      mapVinaMap.addOnMapClickListener(mapRouteClickListener);
  ...
  private void removeListeners() {
    if (isMapClickListenerAdded) {
      mapVinaMap.removeOnMapClickListener(mapRouteClickListener);
  ...
  private void redraw(Style style) {
    routeArrow = new MapRouteArrow(mapView, mapVinaMap, styleRes);
